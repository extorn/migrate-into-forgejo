...
class MigrationSource(ABC):
    """An implementation for this class should be written for each source system to be migrated into Forgejo.
       Then, the source can be specified in the command args e.g. source=gitlab etc"""
    ...
    def list_system_users(self) -> list[CanonicalSystemUser]:
        """Build and return a List all those individuals who have access to the source control system"""
        pass

    @abstractmethod
    def get_repository_role(self, source_access_level:str) -> ForgejoRepositoryRole:
        """Return the exact Forgejo repository role that matches this string representation of the source system access level.
           A role must always be returned, so if one doesn't exist, you'll need to create one. whether you cache them or
           not is up to you, but recommended (@see list_mapped_forgejo_repository_roles)."""
        pass
    ...
    def get_nearest_repository_role(self, source_access_level:str,
        ...
        """Return the Forgejo repository role that matches this string representation of the source system access level
        based on whether upgrade or downgrade are permitted. The precise logic over prefering closest match or
        downgrade vs upgrade if allowed for example is not defined at present."""
        pass
